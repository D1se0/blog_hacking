/* =============================================================================
   enviroments.js
   Lógica de la sección de Entornos (Environments).
   ...
   Contiene:
     ...
     · initEnviroments()     → Punto de entrada: carga datos y configura la vista
     · createEnviromentCard()→ Genera el HTML de una tarjeta de entorno
     · renderTags()          → Renderiza y conecta los botones de filtrado por tag
     · setupSearch()         → Configura el input de búsqueda
     · filterEnviroments()   → Filtrado combinado (texto + tag)
     · renderEnviroments()   → Renderiza la página actual de tarjetas
...
(function () {
  ...
  let allEnviroments      = []; // Todos los entornos cargados del JSON
  let filteredEnviroments = []; // Subconjunto tras aplicar filtros
  let currentPage         = 1;
  let currentTag          = 'All';
  ...
  /* ===========================================================================
     ...
     JSON de entornos, extrae los tags disponibles y renderiza la vista.
  =========================================================================== */
  ...
  function createEnviromentCard(e) {
    ...
    return `
      <a href="view.html?enviroment=${e.id}"
        ...
        <!-- Imagen de portada del entorno -->
        <div class="post-image-container">
          <img src="enviroments/${e.id}/images/logo.png" alt="${e.title}">
        </div>
  ...
  function filterEnviroments() {
    ...
    filteredEnviroments = allEnviroments.filter(e => {
      ...
      // Filtro de tag: construir lista plana de todos los tags del entorno
      let matchesTag = true;
      if (currentTag !== 'All') {
        const tags = [];
  ...
  function renderEnviroments() {
    ...
    const sorted = [...filteredEnviroments].sort((a, b) => new Date(b.date) - new Date(a.date));
    ...
    if (items.length === 0) {
      grid.innerHTML = `
        <p style="color: var(--color-text-muted); text-align: center; width: 100%; margin: 4rem 0;">
